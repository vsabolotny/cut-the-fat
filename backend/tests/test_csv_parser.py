...
class TestParseAmount:
    ...
    def test_german_thousands_negative_salary(self):
        """-3.500 → 3.500,00 € (Beispiel Gehalt)"""
        assert _parse_amount("-3.500") == Decimal("3500")
    ...
    def test_german_thousands_1090(self):
        """-1.090 → 1090 (Beispiel Mieteinnahme)"""
...
class TestParseDate:
    ...
    def test_german_no_leading_zero_month(self):
        """30.1.2026 (kein führendes Null im Monat) → date(2026, 1, 30)"""
...
class TestExtractCardMerchant:
    def test_abrechnung_karte_extracts_merchant(self):
        ...
        desc = "BEISPIEL BAECKEREI//MUSTERSTADT/DE 03-02-2026T09:00:00 Kartennr. 9999"
    ...
    def test_normal_merchant_unchanged(self):
        ...
        assert _extract_card_merchant("REWE", "REWE Markt") == "REWE"
    ...
    def test_pos_zahlung_extracts_merchant(self):
        ...
        desc = "MUSTER RESTAURANT//MUSTERSTADT/DE 03-02-2026T12:00:00"
        assert _extract_card_merchant(merchant, desc) == "MUSTER RESTAURANT"

    def test_case_insensitive_placeholder(self):
        """Placeholder-Erkennung ist case-insensitiv."""
        merchant = "abrechnung karte"
        desc = "BEISPIEL SHOP//MUSTERSTADT/DE 24-02-2026T08:00:00"
        assert _extract_card_merchant(merchant, desc) == "BEISPIEL SHOP"
...
class TestParseCsv:
    ...
    def test_credit_transaction_parsed(self):
        ...
        txns = parse_csv(csv)
        assert len(txns) == 1
        assert txns[0].amount == Decimal("3500")
    ...
    def test_card_merchant_extracted_from_description(self):
        ...
        csv = self._make_csv(
            ["2026-02-12;ABRECHNUNG KARTE;BEISPIEL BAECKEREI//MUSTERSTADT/DE 12-02-2026T09:00:00 Kartennr. 5356;-5,60"],
            "Buchungstag;Begunstigter Auftraggeber;Verwendungszweck;Betrag",
        )
        txns = parse_csv(csv)
        assert len(txns) == 1
    ...
    def test_multiple_transactions(self):
        ...
        txns = parse_csv(csv)
        assert len(txns) == 3
        assert txns[2].amount == Decimal("3500")
    ...
    def test_card_payment_abroad(self):
        """Kartenzahlung im Ausland (.AT) wird korrekt geparst."""
        csv = self._make_csv(
    ...
    def test_date_no_leading_zeros_january(self):
        """30.1.2026 (kein führendes Null) wird korrekt als Januar geparst."""
        csv = self._make_csv(
            ["30.1.2026;Muster Behoerde;Zahlung;-1.090"],
            "Buchungstag;Begunstigter Auftraggeber;Verwendungszweck;Betrag",
        )
        txns = parse_csv(csv)
