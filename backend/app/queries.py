"""
Pure async data-query functions.

Return plain dicts/lists — no ANSI, no Rich, no formatting.
Used by both cli/db.py (via asyncio.run) and web/data.py (async native).
"""
import hashlib
...
async def get_latest_month() -> str | None:
    async with AsyncSessionLocal() as db:
...
async def get_comparison(month: str) -> dict:
...
async def get_insights_data(force: bool = False) -> dict:
...
async def get_uncategorized_merchants() -> list[dict]:
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            text("""
...
async def apply_rule(merchant_normalized: str, category: str) -> int:
...
async def get_ai_suggestions(merchants: list[str], categories: list[str]) -> dict[str, str]:
...
async def get_history(months: int) -> dict:
...
async def ingest_file(filepath: str, progress_cb=None) -> dict:
    import shutil
    ...
    async with AsyncSessionLocal() as db:
        ...
        upload_obj = Upload(filename=filename, file_hash=file_hash, status="processing")
        db.add(upload_obj)
        await db.flush()
        ...
        for txn, norm, dedup, cat, src in with_rule:
            all_txns.append(Transaction(
                upload_id=upload_obj.id, date=txn.date, merchant=txn.merchant,
        ...
        db.add_all(all_txns)
        upload_obj.row_count = len(all_txns)
        upload_obj.status = "done"
        await db.commit()
